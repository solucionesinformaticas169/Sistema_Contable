...
const STORAGE_KEY_PREFIX = "ventas-pedidos-draft";
...
function createDefaultForm(
  empresa: VentasPedidosPanelProps["empresa"],
): PedidoForm {
  return {
    numero: "PED-000001",
    fecha: "09/08/2026",
    ...
    codigoProducto: "",
    observacion: `Pedido generado para ${empresa.razonSocial}.`,
...
export function VentasPedidosPanel({ empresa }: VentasPedidosPanelProps) {
  ...
  const [form, setForm] = useState<PedidoForm>(() => createDefaultForm(empresa));
  const [savedSnapshot, setSavedSnapshot] = useState<PedidoForm>(() =>
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(savedDraft) as PedidoForm;
  ...
  function updateField<K extends keyof PedidoForm>(field: K, value: PedidoForm[K]) {
  ...
  function handleNuevo() {
    ...
    setBanner("Nuevo pedido listo para editar.");
  }

  function handleModificar() {
    setIsEditing(true);
    setBanner("Modo edicion activado para el pedido.");
  }
  ...
  function handleGuardar() {
    ...
    setIsEditing(false);
    setBanner("Pedido guardado localmente en este navegador.");
  }

  function handleCancelar() {
    setForm(savedSnapshot);
    setIsEditing(false);
    setBanner("Se restauraron los datos guardados del pedido.", "warning");
  }
  ...
  function handleEliminar() {
    if (selectedRowId) {
      ...
      setBanner("Se elimino la fila seleccionada del pedido.", "warning");
      return;
    }

    const emptyForm = createDefaultForm(empresa);
    setForm(emptyForm);
    setBanner("Se limpiaron los datos del pedido.", "warning");
  }
  ...
  function handleBuscarProducto() {
    ...
    setBanner(`Producto agregado al pedido: ${nextProduct.descripcion}.`);
  }

  function handleExistencias() {
    setBanner("Consulta de existencias preparada para integrarse con inventario.", "warning");
  }

  function handleInformacion() {
    setBanner(
      form.prioridad
        ? "Pedido marcado como prioritario para seguimiento inmediato."
        : "Pedido en prioridad normal.",
    );
  }
  ...
  function handleAddRow() {
    ...
    setIsEditing(true);
    setBanner("Se agrego una nueva linea al pedido.");
  }
  ...
  return (
    <div className="rounded-sm border border-slate-300 bg-white p-3 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
      <div className="flex flex-wrap items-center gap-2">
        {[
          { label: "Buscar", onClick: () => setBanner("Busqueda de pedidos lista para conectar.") },
          ...
          { label: "Imprimir", onClick: () => { window.print(); setBanner("Se envio el pedido a impresion."); } },
      ...
      {showTutorials ? (
        <div className="mt-3 rounded-sm border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-7 text-slate-600">
          Esta pantalla permite preparar pedidos comerciales con prioridad, cliente,
          detalle editable, observacion y totales recalculados localmente.
        </div>
      ) : null}
      ...
      {showOptions ? (
        <div className="mt-3 rounded-sm border border-slate-200 bg-[#f8fbff] px-4 py-3">
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => updateField("prioridad", !form.prioridad)}
              className="rounded-sm border border-[#cfe0f2] bg-white px-4 py-2 text-sm text-slate-700 transition hover:bg-slate-50"
            >
              Alternar prioridad
            </button>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.08fr_0.92fr]">
        <div className="grid gap-3">
          <div className="grid gap-3 md:grid-cols-[72px_1fr_84px_1fr_120px] md:items-center">
            <label className="text-sm text-slate-700">Pedido</label>
            <input
            ...
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input
                ...
                disabled={!isEditing}
                onChange={(event) => updateField("prioridad", event.target.checked)}
              />
              Prioridad
            </label>
          </div>
          ...
          <div className="mt-2 grid gap-x-3 gap-y-1 md:grid-cols-2">
            {[
              ...
              [
                "Descuento %:",
                totals.subtotal > 0
                  ? formatAmount((totals.descuentoValor / totals.subtotal) * 100, 4)
                  : "0,0000",
              ],
